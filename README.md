...
A friendly macOS menu bar app that lets you choose which **games or apps** get **Game Mode** when they’re fullscreen and in front—whether that’s native Mac games, CrossOver (Wine) games, Steam, or any other app you want to prioritize. Pick what you use, and Game Mode turns on when that app is fullscreen and frontmost, and off when you switch away (same idea as Apple’s built-in rule, but for whatever you want).

## Screenshots

| Menu bar icon | Main window — Mac Apps & CrossOver |
|---------------|-------------------------------------|
| ![Menu bar icon (green when Game Mode is on)](assets/menu-bar-icon.png) | ![Main window: Status, Mac Apps, and CrossOver sections](assets/main-window.png) |

**Keyboard & Settings** — Modifier key swap, CrossOver process list, and app settings.

![Keyboard (Modifier Keys), process selection, and Settings](assets/keyboard-settings.png)
...
- **Accessibility** permission (for fullscreen detection)
...
6. **Settings** — **Start GameMode4All at login**, **Reopen setup**, debug logging, and **Uninstall…**.

Game Mode is turned **on** when a selected app is fullscreen and frontmost, and **off** when it is not.
...
- **Accessibility** — required for fullscreen detection. App Sandbox is disabled (needed for `hidutil`).
...
- **GameModeController.swift** — Runs `gamepolicyctl`, observes apps, fullscreen check.
