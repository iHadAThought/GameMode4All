...
//  Recording uses a local monitor (no Accessibility); the global shortcut requires Accessibility.
//  We do not prompt for Accessibility here—the setup checklist guides users.
//
...
final class GameModeHotKeyManager: ObservableObject {
    ...
    /// Enters recording mode: the next key press in our app becomes the shortcut.
    /// Uses a local monitor (no Accessibility needed). Keep the GameMode4All window focused and press your desired combo.
    func startRecording() {
        ...
        recordingMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            ...
            Task { @MainActor in
                ...
                if let mon = self.recordingMonitor {
